...
.then(emails => {
  ...
  emails.forEach((email) => {
    ...
    main.querySelector(".card-header").addEventListener("click", () => open_email(email["id"]))

    if (mailbox === 'inbox') {
      const archiveButton = document.createElement("button");
      archiveButton.textContent = "Archive";
      archiveButton.className = "btn btn-primary"
      archiveButton.addEventListener("click", () => archive_mail(email.id));
      main.querySelector(".card-body").appendChild(archiveButton);
    }
    else if (mailbox === 'archive') {
      const archiveButton = document.createElement("button");
      archiveButton.textContent = "Unarchive";
      archiveButton.className = "btn btn-primary"
      archiveButton.addEventListener("click", () => unarchive_mail(email.id));
      main.querySelector(".card-body").appendChild(archiveButton);
    }
...
.then(result => {
    ...
    if (result.error) {
      ...
      messagediv.innerHTML = `<h5 class="alert alert-danger" role="alert">${result.error}</h5>`;
      document.querySelector('#compose-view').appendChild(messagediv)
      setTimeout(() => {
      ...
      let messagediv = document.createElement("div")
      messagediv.innerHTML = `<h5 class"alert alert-success" role="alert">${result.message}</h5>`
      document.querySelector('#compose-view').appendChild(messagediv)
      setTimeout(() => {
        messagediv.innerHTML = "";
        load_mailbox('sent');
...
.then(email => {
    ...
    document.querySelector('#email-view').innerHTML = `
      <h5>${email.subject}</h5><p style="float: right;">${email.timestamp}</p><hr><h6>From: ${email.sender}<h6>
